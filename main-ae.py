...
#         self.conv = nn.Conv1d(asp_dim, asp_dim, kernel_size=5, stride=1, padding=2)
#         self.reset_params()
#
...
class DeconvolutionDecoder(nn.Module):
    def __init__(self, embedding, latent_size, len_1, len_2, filter_size, filter_shape=5, tau=.01):
        ...
        voca_size = len(TEXT.vocab)
        self.adaptive_softmax = nn.AdaptiveLogSoftmaxWithLoss(EMBEDDING_DIM, voca_size,
                                                              [round(voca_size / 15), 3 * round(voca_size / 15)])
...
for i_epoch in range(1, N_EPOCHS + 1):
    ...
    for x, y in progress_bar:
        ...
        with torch.no_grad():
            ...
            asd = [[i for i in j if i != 1] for j in reconstruction.argmax(1).detach().cpu().numpy().tolist()]
            asd2 = [[i for i in j if i != 1] for j in x.detach().cpu().numpy().tolist()]
            b = sum([np.round(nltk.translate.bleu([i], j), 5) for i, j in zip(asd, asd2)])
        ...
        progress_bar.set_description(
            "[ TRN LSS: {:.3f} ACC: {:.3f} BLEU: {:.3f} ]".format(*metrics)
        )
    ...
    for x, y in progress_bar:
        ...
        metrics = (
            ...
            accu_total_test / total_test,
            bleu_total_test / total_test
        )
        progress_bar.set_description(
            "[ TST LSS: {:.3f} ACC: {:.3f} BLEU: {:.3f} ]".format(*metrics)
        )
