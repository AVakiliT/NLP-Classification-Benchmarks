...
inputs = Field(lower=True, tokenize='spacy', fix_length=MAX_LEN, pad_first=True, truncate_first=True, batch_first=True)
...
class DiSAN(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        mask_2d = (mask.unsqueeze(1).__and__(mask.unsqueeze(2))).unsqueeze(-1)  # LL1
...
models = [
    ...
    SNLI(Lstm, 2 * EMBEDDING_DIM).to(device)
]
